...
from lattice.core.errors import ConfigurationError
...
def _create_llm_provider(config: ProviderConfig) -> BaseLLMProvider:
    ...
    if provider_name == "openai":
        from lattice.providers.openai_provider import OpenAILLMProvider
        return OpenAILLMProvider(config)

    elif provider_name == "ollama":
        from lattice.providers.ollama_provider import OllamaLLMProvider
        return OllamaLLMProvider(config)

    elif provider_name == "anthropic":
        from lattice.providers.anthropic_provider import AnthropicLLMProvider
        return AnthropicLLMProvider(config)

    elif provider_name == "google":
        from lattice.providers.google_provider import GoogleLLMProvider
...
def _create_embedding_provider(config: ProviderConfig) -> BaseEmbeddingProvider:
    ...
    if provider_name == "openai":
        from lattice.providers.openai_provider import OpenAIEmbeddingProvider
        return OpenAIEmbeddingProvider(config)

    elif provider_name == "ollama":
        from lattice.providers.ollama_provider import OllamaEmbeddingProvider
        return OllamaEmbeddingProvider(config)

    elif provider_name == "google":
        from lattice.providers.google_provider import GoogleEmbeddingProvider
        return GoogleEmbeddingProvider(config)

    elif provider_name == "unixcoder":
        from lattice.providers.unixcoder_provider import UniXcoderEmbeddingProvider
